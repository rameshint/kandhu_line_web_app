<?php
// Firebase sync handler using Realtime Database - separate file for AJAX calls
session_start();
require_once 'vendor/autoload.php';

use Kreait\Firebase\Factory;

class FirebaseMySQLSyncRealtime
{
    private $firebase;
    private $database;
    private $pdo;
    ...
    {
        // Initialize Firebase (no gRPC required for Realtime Database)
    ...
    public function syncCustomersToFirebase($line = 'Daily')
    {
        try {
            ...
            $customersSynced = 0;
            $customersRef = $this->database->getReference('customers');
            ...
            foreach ($customers as $customer) {
                ...
                // Store using customer_no as key
                $customersRef->getChild($customer['customer_no'])->set($customerData);
                $customersSynced++;
            }
    ...
    public function syncLoansToFirebase($loanType = 'Daily')
    {
        try {
            ...
            $loansSynced = 0;
            $loansRef = $this->database->getReference('loans');
            ...
            foreach ($loans as $loan) {
                ...
                $loanData = [
                    ...
                    'loanOpeningDate' => date('c', strtotime($loan['loanOpeningDate'])),
                    'loanClosingDate' => date('c', strtotime($loan['loanClosingDate']))
                ];

                // Store using loanId as key
                $loansRef->getChild($loanId)->set($loanData);
                $loansSynced++;
            }
    ...
    public function syncCollectionsFromFirebase()
    {
        try {
            ...
            $collections = $collectionsRef->orderByChild('syncFlag')->equalTo(false)->getValue();
            ...
            if (!$collections) {
                return [
                    ...
                    'errors' => 0,
                    'message' => 'No new collections to sync'
                ];
            }
            ...
            foreach ($collections as $collectionId => $data) {
                try {
                    ...
                    $collectionDate = date('Y-m-d', strtotime($data['timestamp']));
                    $collectionTime = date('H:i:s', strtotime($data['timestamp']));
                    ...
                    $collectionsRef->getChild($collectionId)->update([
                        'syncFlag' => true,
                        'syncedAt' => date('c')
                    ]);
...
if (isset($_POST['action']) || isset($_GET['action'])) {
    ...
    try {
        $sync = new FirebaseMySQLSyncRealtime();
